...
\begin{figure}[t]
\centering
\fbox{\rule{0pt}{1.6in}\rule{0.9\columnwidth}{0pt}}
\caption{Per-class F1 evolution over 200 training epochs on MIT-BIH (LSTM architecture). AdaCAL (solid lines) continues improving minority class F1 after epoch 100, while static baselines plateau. Minority classes (3, 4) benefit most from adaptive reweighting.}
...
\centering
\fbox{\rule{0pt}{1.6in}\rule{0.9\columnwidth}{0pt}}
\caption{Training (dashed) and validation (solid) macro-F1 convergence curves on ElectricDevices across three architectures. AdaCAL achieves higher final validation macro-F1 than all static baselines while converging at comparable speed.}
